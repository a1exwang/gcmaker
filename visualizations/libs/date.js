...
//
// The other significant difference from the built-in JavaScript Date is that `timezoneJS.Date` also has named properties that store the values of year, month, time, etc., so it can be directly serialized to JSON and used for data transfer.
...
(function () {
  ...
  //
  // This is used to pad numbers in converting time to string in ISO standard.
  ...
  timezoneJS.Date = function () {
    ...
    //
    // - `year, month, [time,] [hours,] [minutes,] [seconds,] [millis,] [tz]: Same as `Date` object
    // with tz.
    //
    ...
    if (isNaN(dt.getTime())) { // invalid time were passed
    ...
    // Tricky part:
    // The time is either given as unambiguous UTC time or otherwise the time is assumed
    // to be a time in timezone `tz` or a locale time if `tz` is not provided. Thus, to
    ...
    //
    // `dt_str_tz` is a time string containing timezone information, i.e. containing 'Z', 'T' or
    // /[+-][0-9]{4}/ (e.g. '+0200'), while `dt_str` is a string which does not contain
    // timezone information. See: http://dygraphs.com/time-formats.html
    if (is_dt_local) {
  ...
  timezoneJS.timezone = new function () {
    ...
    //if isUTC is true, time is given in UTC, otherwise it's given
    // in local time (ie. time.getUTC*() returns local time components)
    function getRule(dt, zone, isUTC) {
      ...
      //Convert a time to UTC. Depending on the 'type' parameter, the time
      // parameter may be:
      //
      ...
      //
      //Step 2:  Sort the rules by effective time.
      //
      //Step 3:  Check requested time to see if a rule has yet taken effect this year.  If not,
      //
      ...
      //
      //Step 5:  Sort the rules by effective time.
      //Step 6:  Apply the most recent rule before the current time.
      var convertRuleToExactDateAndTime = function (yearAndRule, prevRule) {
        var year = yearAndRule[0]
          , rule = yearAndRule[1];
          // Assume that the rule applies to the year of the given time.
      ...
      // As the given time may indeed be close to a DST change, it may get sorted
